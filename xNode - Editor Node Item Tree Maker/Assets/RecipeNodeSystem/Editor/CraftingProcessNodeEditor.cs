...
public class CraftingProcessNodeEditor : NodeEditor {
	...
	public const float topBeginningOffset = 78f;
	public const float portFieldVerticalSize = 18f;
	...
	public override void OnBodyGUI () {
		...
		for (int i = 0; i < node.inputItemCounts.Count; i++) {
			if (GUI.Button(new Rect(100, topBeginningOffset + (i * portFieldVerticalSize), 17, 17), "-")) {
				node.inputItemCounts[i]--;
			}
			if (GUI.Button(new Rect(100+18, topBeginningOffset + (i * portFieldVerticalSize), 17, 17), "+")) {
				node.inputItemCounts[i]++;
			}
		...
		for (int i = 0; i < node.outputItemCounts.Count; i++) {
			...
			if (GUI.Button(new Rect(65, topBeginningOffset + (i * portFieldVerticalSize) + inputOffset, 17, 17), "-")) {
				node.outputItemCounts[i]--;
			}
			if (GUI.Button(new Rect(65 + 18, topBeginningOffset + (i * portFieldVerticalSize) + inputOffset, 17, 17), "+")) {
				node.outputItemCounts[i]++;
			}
	...
	public int GUIIntFieldInput (int input, int offset) {
		string s = GUI.TextField(new Rect(65, topBeginningOffset+(offset* portFieldVerticalSize), 30, 17), input.ToString());
		int output = 1;
	...
	public int GUIIntFieldOutput (int input, int offset, float inputOffset) {
		string s = GUI.TextField(new Rect(105, topBeginningOffset + (offset * portFieldVerticalSize) + inputOffset, 30, 17), input.ToString());
		int output = 1;
		if (int.TryParse(s, out output)) {
			return output;
		} else {
